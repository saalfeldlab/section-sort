...
public class TSPTest {
	...
	// Concorde must be present in either some directory in PATH env variable
	// or in <IMAGEJ_APP_DIR>/bin/.  <IMAGEJ_APP_DIR> can be made available to maven
	// by calling it e.g.
	//
	// mvn -Dimagej.app.directory=$HOME/packages/Fiji.app/
	//
	// which also installs the artifact in <IMAGEJ_APP_DIR>/jar
	// Alternative: set PATH variable in eclipse by
	// Run -> Run Configurations -> Environment -> New { "Variable" : "PATH", "Value" : "/path/to/directory/containing/concorde" }
	private String imageJAppDir;
	{
        imageJAppDir = System.getProperty( "imagej.app.directory" );
        imageJAppDir = imageJAppDir == null ? "" : imageJAppDir + "bin/";
        System.out.println( "ImageJAppDir = " + imageJAppDir );
	}
	final String concordeExecutablePath = imageJAppDir + "concorde";
